...
import org.kokho.scheduling_new.multicritical.schedulers.MulticriticalScheduler
import org.kokho.scheduling_new.multicritical.system.MulticriticalTask
import org.kokho.scheduling_new.utils.SchedulerAnalyzer
import org.kokho.scheduling_new.{Job, ScheduledJob, SchedulerBehavior}
...
trait MulticriticalSchedulerBehavior extends SchedulerBehavior{
  ...
  implicit def tupleToSchedule(tuple: Product): MulticriticalScheduler = {
    val partition = tuple.productIterator.map(_.asInstanceOf[Seq[MulticriticalTask]])
    toScheduler(partition.toSeq)
  }

  implicit def seqToSchedule(seq: Seq[MulticriticalTask]): MulticriticalScheduler =
    toScheduler(Seq(seq))

  implicit def partitionToSchedule(partition: Partition): MulticriticalScheduler =
