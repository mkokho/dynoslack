...
import org.kokho.scheduling_new.multicritical.system.{HiCriticalTask, LoCriticalTask}
import org.kokho.scheduling_new.utils.SchedulerAnalyzer
import org.kokho.scheduling_new.{Job, ScheduledJob}
...
with MulticriticalSchedulerBehavior {
  ...
  it should "not release an early job if the task is active on another processor" in {
    ...
    val analyzer = new SchedulerAnalyzer(partition, 15, true)

    analyzer.taskToJobs(taskL) should contain (taskL.shift(4).job(0))
