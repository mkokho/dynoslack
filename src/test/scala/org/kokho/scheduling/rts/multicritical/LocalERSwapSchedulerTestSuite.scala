...
class LocalERSwapSchedulerTestSuite extends FunSuite{
  ...
  private def checkEarlyReleaseInThe2ndPeriod(tasks: Seq[MulticriticalTask]): Unit = {
    ...
    val firstJobs = Set(hiTask.job(1), loTask.job(1))
    ...
    val loTaskER = loTask.releaseEarlyJob(period + loTask.earlyReleases.head)
    val jobs2ndPeriod = Set(hiTask.job(2), loTask.job(2), loTaskER.job(1))
  ...
  test("testing local early release without static slack") {
    val hiTask = new HiCriticalTask(6, 2, 4, isOdd)
  ...
  test("testing local early release with static slack") {
    val hiTask = new HiCriticalTask(7, 3, 4, isOdd)
  ...
  test("testing local early release of two jobs at the same time") {
    val hiTask = new HiCriticalTask(10, 2, 6, isOdd)
    ...
    val firstJobs:Set[Job] = Set(hiTask.job(1), loTaskA.job(1), loTaskB.job(1))
    val firstScheduledJobs = schedule.take(10).map(_.job).toSet

    assert(firstJobs == firstScheduledJobs, "First jobs of the tasks has not been scheduled correctly")

    val loTaskA_ER = loTaskA.releaseEarlyJob(14)
    val loTaskB_ER = loTaskB.releaseEarlyJob(14)
    val jobs2ndPeriod = Set(hiTask.job(2), loTaskA.job(2), loTaskB.job(2), loTaskA_ER.job(1), loTaskB_ER.job(1))
